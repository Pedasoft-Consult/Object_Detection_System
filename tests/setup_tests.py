...
def setup_environment():
    ...
    # Check if setup_tests.py exists
    if os.path.exists("setup_tests.py"):
        result = subprocess.run(
            [sys.executable, "setup_tests.py"],
            capture_output=True,
            text=True
        )

        if result.returncode != 0:
            logger.error(f"Error setting up test environment: {result.stderr}")
            return False

        logger.info(result.stdout)
    else:
        logger.warning("setup_tests.py not found, skipping environment setup")
...
def run_tests(output_dir="test_results", api_url="http://localhost:8000", skip_api=False,
    ...
    if os.path.exists("run_tests.py"):
        ...
        if result.returncode != 0:
            logger.warning(f"Test runner returned non-zero exit code: {result.returncode}")
            # Return True anyway to continue processing
            return True
